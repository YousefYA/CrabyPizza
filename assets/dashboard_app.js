...
crustDivs.forEach((div) => {
  div.addEventListener("click", (e) => {
    document
      .querySelector(".selected-crust")
      .classList.remove("selected-crust");
...
const openGenre = (e, genre) => {
  ...
  document
    .querySelector(".toppings-body-child.active-genre")
    .classList.remove("active-genre");

  genre === "veg"
    ? document.querySelector(".toppings-body-veg").classList.add("active-genre")
    : document
        .querySelector(".toppings-body-non")
        .classList.add("active-genre");
};
...
const updateImage = (toppings) => {
  document
    ...
    .forEach((img) => img.classList.remove("yes"));
...
function calculateCost($toppings, $crust, $quantity, $extraCheese) {
  // Example calculation logic
  $basePrice = 10; // base price per pizza
  $toppingsCost = count($toppings) * 2; // assume each topping costs $2
  $cheeseCost = $extraCheese ? 1 : 0;
  return ($basePrice + $toppingsCost + $cheeseCost) * $quantity;
}
